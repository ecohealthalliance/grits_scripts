#Script to match grid event metadata ( event, location, year )
#in title 
...
def main():
    ...
    for file_name in filelist:
		...
		#Verify if article has doi. If not, 
		...
		#Initialize variable to store matched terms
		terms_present = defaultdict(list)
                num_match = [0] * len(event_term_list)

		#Iterate through GRID event terms and search for 
		#them in titles. If present, update variables.
                for id_i, terms in event_term_list.items():
                    for val in terms:
                        val = '\\b(' + val.lower() + ')\\b'
                        match = re.search(val, title)
